...
- `docs/DESIGN_RULES.md` is the binding implementation contract for the
  blurrf product. Keep code, tests, and future design changes consistent with
  it; update the document when an intentional contract changes.
...
- Root causes that required multi-step debugging.
*(Do not record trivial typos, transient network glitches, or standard iterative code changes.)*
